#!/usr/bin/env bash
# COMP2137 System Report Script
# Purpose: Produce a labeled, human-readable system report using dynamic data only.
# Concepts demonstrated (per course notes):
# - Shebang, comments, templated output                 (#/env bash; comments; heredoc)
# - Variables + command substitution                     (store data first, then print)
# - Pipelines (gather | filter | manipulate | format)    (ip/ss/awk/sort/paste pipelines)
# - Use of admin tools (ip, lshw, df, free, who, ps, wc, ss, ufw) and sudo where needed
...
# OS (source /etc/os-release per rubric)
OS_NAME="Unknown"
...
# CPU (hardware needs root; try lshw first)
CPU="Unknown"
if have lshw; then
...
# Disks (make/model/size via lshw; one per line)
DISKS="(none detected)"
if have lshw; then
...
# ----- System Status -----
# Logged-in users (who | uniq; pipeline: gather->filter->format)
...
# ============================ Output =================================
# Requirement: blank line before and after the report for readability.
echo
cat <<REPORT
